...
class Array : public ArrayBase<Array<T>, T> {
 public:
  ...
  constexpr Array& operator=(Array&& a) noexcept {
   ...
   constexpr Array& operator=(const Array& x) noexcept {
//    assert(n == x.n);
    if (n!=x.n) {
       if (data_) delete [] data_;
       data_= new T[x.n];
  ...
  template <class A_t, typename other_type>
   constexpr Array& operator=(const ArrayBase<A_t, other_type>& a) noexcept {
    assert(n == a.size());
    for (auto i = 0; i < n; ++i) data_[i] = a[i];
    return *this;
  }
  ...
  template <class Xpr_t>
   constexpr Array& operator=(const ArrayXpr<Xpr_t>& a) noexcept {
    for (auto i = 0; i < n; ++i) {
      data_[i] = a[i];
    }
    return *this;
   ...
   constexpr Array& operator=(const value_t& x) noexcept {
  ...
  template <class Xpr_t>
   constexpr Array& operator+=(const ArrayXpr<Xpr_t>& x) noexcept {
  ...
  template <class A_t>
   constexpr Array& operator+=(const ArrayBase<A_t, value_t>& x) noexcept {
    for (auto i = 0; i < n; ++i) data_[i] += x[i];
    return *this;
  }
  template <class A_t>
  ...
  template <class A_t>
   ...
   constexpr pointer_t begin() noexcept { return data_; }
   constexpr const_pointer_t begin() const noexcept { return data_; }
   constexpr const_pointer_t end() const noexcept { return (data_ + n); }
   constexpr pointer_t rbegin() noexcept { return (data_ + n); }
   constexpr const_pointer_t rbegin() const noexcept { return (data_ + n); }
   constexpr const_pointer_t rend() const noexcept { return data_; }
   constexpr const_pointer_t data() const noexcept { return data_; }
   constexpr pointer_t data() noexcept { return data_; }
   ...
   constexpr value_t& operator[](size_type i) noexcept { return data_[i]; }
   constexpr value_t operator[](size_type i) const noexcept { return data_[i]; }

   constexpr value_t norm() noexcept {
    value_t sum(0);
...
class SubArray : public ArrayBase<SubArray<T>, T> {
 public:
  ...
  ///// various operators
  constexpr SubArray& operator=(SubArray&& a) noexcept {
    data_ = a.data_;
    a.data_ = nullptr;
    n = a.n;
    a.n = 0;
  ...
  constexpr SubArray& operator=(const SubArray& x) noexcept {
    assert(n == x.n);
    data_ = x.data_;
    n = x.n;
    return *this;
   ...
   constexpr SubArray& operator=(const Array<T>& x) noexcept {
    assert(n == x.n);
    data_ = x.data();
    n = x.size();
    return *this;
  ...
  template <class A_t, typename other_type>
   constexpr SubArray& operator=(const ArrayBase<A_t, other_type>& a) noexcept {
    assert(n == a.size());
    for (auto i = 0; i < n; ++i) data_[i] = a[i];
    return *this;
  }
  ...
  template <class Xpr_t>
   constexpr SubArray& operator=(const ArrayXpr<Xpr_t>& a) noexcept {
    assert(n == a.size());
    for (auto i = 0; i < n; ++i) data_[i] = a[i];
    return *this;
  }

   constexpr SubArray& operator=(const value_t& x) noexcept {
    for (auto i = 0; i < n; ++i) data_[i] = x;
    return *this;
  }
  ...
  template <class Xpr_t>
   constexpr SubArray& operator+=(const ArrayXpr<Xpr_t>& x) noexcept {
    for (auto i = 0; i < n; ++i) data_[i] += x[i];
    return *this;
  }
  template <class Xpr_t>
  ...
  template <class A_t>
   constexpr SubArray& operator+=(const ArrayBase<A_t, value_t>& x) noexcept {
    for (auto i = 0; i < n; ++i) data_[i] += x[i];
    return *this;
  }
  template <class A_t>
  ...
  template <class A_t>
   ...
   constexpr const_pointer_t begin() const noexcept { return data_; }
   constexpr const_pointer_t end() const noexcept { return (data_ + n); }
   constexpr pointer_t rbegin() noexcept { return (data_ + n - 1); }
   constexpr const_pointer_t rbegin() const noexcept { return (data_ + n - 1); }
   constexpr const_pointer_t rend() const noexcept { return (data_); }
   constexpr const_pointer_t data() const noexcept { return data_; }
   constexpr pointer_t data() noexcept { return data_; }
   constexpr bool empty() noexcept { return n == 0; }
...
class SliceArray : public ArrayBase<SliceArray<T>, T> {
 public:
  ...
  ///// various operators
   constexpr SliceArray& operator=(SliceArray&& a) noexcept {
    data_ = a.data_;
    a.data_ = nullptr;
    n = a.n;
    a.n = 0;
   ...
   constexpr SliceArray& operator=(const SliceArray& x) noexcept {
    assert(n == x.n);
    data_ = x.data_;
    n = x.n;
    str = x.str;
   ...
   constexpr SliceArray& operator=(const Array<T>& x) noexcept {
    assert(n == x.n);
    data_ = x.data();
    n = x.size();
  ...
  template <class Xpr_t>
   constexpr SliceArray& operator=(const ArrayXpr<Xpr_t>& a) noexcept {
    assert(n == a.size());
    std::size_t i(0);
    std::ptrdiff_t ix(0);
    for (;i<n;++i,ix+=str) {
   ...
   constexpr SliceArray& operator=(const value_t& x) noexcept {
    std::size_t i(0);
    std::ptrdiff_t ix(0);
    for (;i<n;++i,ix+=str) {
      data_[ix] = x;
  ...
  template <class Xpr_t>
   constexpr SliceArray& operator+=(const ArrayXpr<Xpr_t>& x) noexcept {
    std::size_t i(0);
    std::ptrdiff_t ix(0);
  ...
  template <class A_t>
   constexpr SliceArray& operator+=(const ArrayBase<A_t, value_t>& x) noexcept {
    std::size_t i(0);
    std::ptrdiff_t ix(0);
    for (;i<n;++i,ix+=str) {
      data_[ix] += x[i];
  ...
  constexpr iterator_t begin() noexcept { return iterator_t(data_, str); }
  ...
  constexpr bool empty() const noexcept { return n == 0; }
   ...
   constexpr value_t& operator[](size_type i) noexcept { return data_[i * str]; }
  ...
  constexpr T norm() const noexcept {
    ...
    std::ptrdiff_t ix(0);
    std::size_t i(0);
    for (;i<n;++i,ix+=str) sum += data_[ix] * data_[ix];
    return sum;
  }
...
constexpr typename promote_traits<x_t, y_t>::promote_t dotProduct(
